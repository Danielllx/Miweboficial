// Accessibility controls removed (buttons + handlers) per user request
...
document.querySelectorAll('.btn-primary, .btn-secondary').forEach(button => {
    if (!button.closest('form') && button.textContent.includes('Comenzar')) {
        ...
        button.addEventListener('click', (e) => {
            e.preventDefault();
            const gallerySection = document.querySelector('#proyectos');
            if (gallerySection) {
                const headerOffset = 80;
...
// Firebase (Firestore) client-side + reCAPTCHA v3
// He usado imports modulares desde el CDN. Tu firebaseConfig fue pegado arriba en el archivo.
(function () {
    // Primero, insertar el script de reCAPTCHA (site key proporcionada por el usuario)
...
if (contactForm) {
    contactForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            // Success UI: toast, confetti, button check
            try {
...
window.addEventListener('scroll', () => {
    ...
    // Efecto parallax en hero: solo en pantallas grandes y no táctiles
    const hero = document.querySelector('#inicio');
    const touchDevice = ('ontouchstart' in window) || navigator.maxTouchPoints > 0;
    if (hero && !touchDevice && window.innerWidth > 768 && currentScrollY < window.innerHeight) {
        hero.style.transform = `translateY(${currentScrollY * 0.5}px)`;
        hero.style.opacity = 1 - (currentScrollY / window.innerHeight) * 0.5;
    } else if (hero) {
        // Reset transforms on touch/small devices to avoid visual glitches
...
const observer = new IntersectionObserver((entries) => {
    entries.forEach((entry, index) => {
        if (entry.isIntersecting) {
            setTimeout(() => {
                // Always fade in (opacity) but only apply move when appropriate
                entry.target.classList.add('animate-in');
                entry.target.classList.remove('pre-animate');

                // If the element is an image container and device is non-touch/large, add float
                if (entry.target.classList.contains('image-container')) {
                    if (!isTouch && window.innerWidth > 768) {
                        entry.target.classList.add('float-animation');
                    }
                } else {
                    // For typical blocks (cards, project items, etc.) add move transform
...
// Observe cards, image containers, section titles and project items so they animate when visible
document.querySelectorAll('.card, .image-container, .section-title, .project-item').forEach(el => {
    // add pre-animate class so CSS handles the initial state
...
// Observar números para animarlos cuando sean visibles
...
// Carrito simple: persistente en localStorage + sidebar. Botones agregan al carrito.
(function () {
    const STORAGE_KEY = 'voltxe_cart_v1';
    let cart = [];
    ...
    function updateBadge() {
        ...
        if (cartCountBadge) cartCountBadge.textContent = count + (count === 1 ? ' item' : ' items');
    }
    ...
    function renderCart() {
        ...
        if (!cart || cart.length === 0) {
            container.innerHTML = '<div style="color:#9CA3AF;padding:12px 0">Tu carrito está vacío</div>';
        ...
        cart.forEach((it, idx) => {
            ...
            thumb.style.width = '56px'; thumb.style.height = '56px'; thumb.style.flex = '0 0 56px'; thumb.style.borderRadius = '8px'; thumb.style.overflow = 'hidden';
            ...
            row.appendChild(thumb); row.appendChild(meta); row.appendChild(priceEl);
    ...
    document.querySelectorAll('.btn-add-to-cart').forEach(btn => {
        if (btn.dataset.cartBound) return; btn.dataset.cartBound = '1';
        btn.addEventListener('click', (e) => { e.preventDefault(); const name = btn.getAttribute('data-product-name') || 'Producto'; const price = btn.getAttribute('data-product-price') || '0'; const img = btn.getAttribute('data-product-image') || ''; addToCart(name, price, img, btn); });
    });
    ...
    // Bind any header/cart-toggle buttons (e.g. #cartButtonNav) to open the sidebar
    document.querySelectorAll('.cart-toggle').forEach(t => {
        if (!t.dataset.cartBound) { t.dataset.cartBound = '1'; t.addEventListener('click', (e)=>{ e.preventDefault(); openCart(); }); }
    });
    ...
    // checkout -> fill contact form and scroll
    document.getElementById('checkoutBtn')?.addEventListener('click', (e)=>{
        e.preventDefault(); if(!cart || cart.length===0){ try{window.showToast('Tu carrito está vacío','info');}catch(e){} return; }
        const productList = cart.map(it => `${it.name}${it.qty && it.qty>1 ? ' x'+it.qty : ''}${it.price?` (${formatPrice(it.price)})`:''}`).join(', ');
        const total = cart.reduce((s,it)=>s + (Number(it.price||0)*(it.qty||1)),0);
        const asuntoInput = document.getElementById('contact-asunto'); if(asuntoInput) asuntoInput.value = `Me interesa: ${productList} — Total: ${formatPrice(Math.round(total*100)/100)}`;
        closeCartFn(); const contactSection = document.querySelector('#contacto'); if(contactSection){ const headerOffset = 80; const elementPosition = contactSection.getBoundingClientRect().top; const offsetPosition = elementPosition + window.pageYOffset - headerOffset; window.scrollTo({ top: offsetPosition, behavior: 'smooth' }); try{window.showToast('Formulario relleno. Completa tus datos para enviar.','info',2200);}catch(e){} }
    });
...
// Chat widget logic removed per user request

// Ensure hero buttons with data-target scroll correctly
